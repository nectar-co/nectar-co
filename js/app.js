 ...
 * Loads catalog data reliably (works offline & online), filters, modals & mobile-first cart interaction
 */
...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  // Cart Drawer & Mobile Sticky Bar Elements
  ...
  const btnMobileStickyOpen = document.getElementById('btnMobileStickyOpen');
  ...
  function renderProducts() {
    ...
    if (state.filteredProducts.length === 0) {
      productsGrid.innerHTML = `
        <div class="empty-state">
          ...
          <h3 style="font-family: var(--font-display); font-size: 1.3rem; margin-bottom: 8px;">No encontramos coincidencias</h3>
          <p style="color: var(--brown-600); margin-bottom: 16px; font-size: 0.9rem;">Prueba buscando por marca (ej: Lattafa, Armaf, Afnan) o limpia los filtros.</p>
          <button class="btn-outline" id="btnResetFilters" style="padding: 8px 18px; font-size: 0.85rem;">Limpiar Filtros</button>
        </div>
      `;
    ...
    productsGrid.innerHTML = state.filteredProducts.map(product => {
      ...
      if (!product.stock || product.stock === 0) {
        stockHtml = `<div class="card-stock-indicator soldout"><span class="stock-dot"></span>Agotado</div>`;
      } else if (product.stock <= 3) {
        stockHtml = `<div class="card-stock-indicator urgent"><span class="stock-dot pulse"></span>¡Solo quedan ${product.stock}!</div>`;
      } else if (product.stock <= 7) {
        stockHtml = `<div class="card-stock-indicator available"><span class="stock-dot"></span>Pocas unidades (${product.stock})</div>`;
      } else {
        stockHtml = `<div class="card-stock-indicator available"><span class="stock-dot"></span>✓ Disponible</div>`;
      }
      ...
      if (product.stock && product.stock <= 2) {
        badgeHtml += `<span class="badge-tag urgent">Últimas</span>`;
      ...
      return `
        <div class="product-card" data-product-id="${product.id}">
          ...
          <div class="card-header-meta">
            ...
            <span class="card-gender-chip">${product.gender === 'unisex' ? 'Unisex' : product.gender === 'hombre' ? 'Él' : 'Ella'}</span>
          </div>

          <h3 class="card-title" data-action="open-modal">${product.name}</h3>
          
          <div class="card-bottle-size">
            ${product.bottleSize ? product.bottleSize.replace('Botella Sellada ', '📦 ') : '📦 100 ml'}
          </div>
      ...
      <div class="modal-product-layout">
        ...
        <div class="modal-info-col">
          ...
          <div class="notes-highlight-box">
            <span style="font-size: 0.78rem; font-weight: 700; color: var(--amber-600); text-transform: uppercase; letter-spacing: 1px; display: block; margin-bottom: 3px;">Acordes & Notas Olfativas:</span>
            <span style="font-size: 0.9rem; color: var(--brown-900); font-weight: 500;">${product.notes || 'Especias orientales, maderas nobles y vainilla.'}</span>
          </div>

          <!-- Price & CTA -->
          <div style="margin-top: 6px; padding-top: 12px; border-top: 1px solid var(--sand-border);">
            <div style="font-family: var(--font-mono); font-size: 1.6rem; font-weight: 800; color: var(--brown-900); margin-bottom: 12px;">
  ...
  function openCartDrawer() {
    ...
    document.body.style.overflow = 'hidden';
    if (mobileStickyCartBar) mobileStickyCartBar.classList.remove('visible');
  }
    ...
    if (items.length === 0) {
      cartItemsList.innerHTML = `
        <div style="text-align: center; padding: 40px 16px;">
          ...
          <p style="font-size: 0.86rem; color: var(--brown-600);">Explora el catálogo y agrega tus perfumes favoritos para generar tu cotización o pedir por WhatsApp.</p>
        </div>
      `;
    ...
    cartItemsList.innerHTML = items.map(item => `
      <div class="cart-item-card" data-key="${item.key}">
        ...
        <div class="cart-item-details">
          ...
          <span class="cart-item-meta">${item.brand} • ${item.bottleSize || '100 ml'}</span>
  ...
  // Update mobile sticky bottom cart pill
  function updateMobileStickyBar() {
    if (!mobileStickyCartBar || !mobileStickyCount || !mobileStickyTotal) return;
    const count = Cart.getCount();
    const total = Cart.getTotal();

    if (count > 0 && !(cartDrawerBackdrop && cartDrawerBackdrop.classList.contains('active'))) {
      mobileStickyCount.textContent = `${count} ${count === 1 ? 'Perfume' : 'Perfumes'}`;
      mobileStickyTotal.textContent = `$ ${new Intl.NumberFormat('es-CO').format(total)} COP`;
      mobileStickyCartBar.classList.add('visible');
    } else {
      mobileStickyCartBar.classList.remove('visible');
    }
  }
  ...
  if (btnMobileStickyOpen) btnMobileStickyOpen.addEventListener('click', openCartDrawer);
